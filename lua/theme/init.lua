-- Fork from luisiacc/gruvbox-baby
local M    = {}
...
local setup_scheme = function(theme, config)
    local config = config or default_config
    local scheme = require("theme.scheme."..theme)
    local c      = scheme.color
    local diff   = scheme.diff

    local theme = {}
    theme.defer = {}
    theme.term = scheme.term
    theme.base =
    {
...
function M.setup(theme, config)
    util.load(setup_scheme(theme, config))
end
