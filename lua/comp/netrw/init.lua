...
netrw =
{
    ...
    -- @param path: The copy target's full path
    -- @return True when insert insert succeed.
    try_copy = function(self, path)
        for i = #self.copied, 1 do
    ...
    -- @param path: The cut target's full path
    -- @return True when insert insert succeed.
    try_cut = function(self, path)
        file_name = vim.fn.fnamemodify(path, ":t")
        for i = 1, #self.cut do
            if self.cut[i] == element then
                return false
            end
        end
    ...
    -- Get marked list
    -- @return marked list as table type
    get_marked_list = function(self)
    ...
    -- Change marked list
    -- @param newlist: new table that need to be marked
    set_marked_list = function(self, newlist)
        if newlist == nil then
            return
        end
        local args = table.concat(newlist, ",")
        vim.cmd([[call netrw#Modify('netrwmarkfilelist',[]]..args..[[])]])
    end,
    ...
    get_cur_path = function(self)
        vim.cmd([[let g:netrw_cur_file_name = netrw#Call('NetrwGetWord')]])
        local path = vim.b.netrw_curdir .. '/' .. vim.g.netrw_cur_file_name
        if path:sub(-1) == "/" then
            path = path:sub(1, -2)
        end
	return path
...
local netrw_copy_file = function()
    netrw:toggle_copy(netrw.get_cur_path())
end

-- Cut file
local netrw_cut_file = function()
    netrw:toggle_cut(netrw.get_cur_path())
end
...
local netrw_open = function()
    ...
    -- If user closed the netrw buf with cli, we should recheck status
    if vim.g.NetrwIsOpen == true then
        local netrw_bufs = nvim.ext.buf.get_bufs_with_var("filetype", "netrw")
    end

    -- The window is opened, we just focus on it
    if vim.g.NetrwIsOpen == true then
        vim.cmd("wincmd w")
    -- Or we are creating a new window
    else
...
local netrw_close_if_last = function()
    if nvim.ext.win.get_cur_wins_count() == 1 and vim.o.filetype == 'netrw' then
        vim.cmd("q")
    end
end

-- On a directory enter
-- @param path_before: The directory path that is before enter a directory
-- @param path_after: The directory path that is the entering directory
local netrw_on_enter_directory = function(path_before, path_after)
    ...
    if netrw.hl[path_before] ~= nil then
        for i, value in pairs(netrw.hl[path_before]) do
            if value.ishl == true then
                print("Delete", path_before)
                vim.fn.matchdelete(value.id)
                value.ishl = false
            end
        end
    ...
    if netrw.hl[path_after] ~= nil then
        for i, value in pairs(netrw.hl[path_after]) do
            if value.ishl == false then
                print("Add", path_after)
...
local netrw_open_file_or_directory = function()
    local cur_dir = vim.b.netrw_curdir
    vim.cmd("normal \r")
    local selected_path = vim.b.netrw_curdir
    if vim.fn.isdirectory(selected_path) ~= 1 then
        return
    end
    netrw_on_enter_directory(cur_dir, selected_path)
end
...
vim.api.nvim_create_autocmd("WinEnter", { pattern = "*",      callback = netrw_close_if_last })

-- Netrw style
vim.cmd("highlight netrwCopied  ctermbg=green")
vim.cmd("highlight netrwCut     ctermbg=red")
vim.cmd("hi! link netrwMarkFile Search")
vim.api.nvim_set_var("netrw_keepdir", 0)                           -- Keep current directory (in the buffer) is matching with the browsing directory.
vim.api.nvim_set_var("netrw_winsize", 15)                          -- Netrw width size
...
vim.api.nvim_set_var("netrw_liststyle", 0)                         -- List style:
